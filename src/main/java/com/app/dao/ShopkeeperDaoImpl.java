...
//	@Override
//	public List<Appointments> appointmetsList(String ownerName,int userId) {
//
//		Shops s=getShopByOwnerName(ownerName);
//		Users u=getUserByUserId(userId);
//		String jpql = "select a from Appointments a INNER JOIN a.users u where shop_id=:sid and a.appoint_id=:userId";
//		List<Appointments> alist=new ArrayList<Appointments>();
//		alist=mgr.createQuery(jpql, Appointments.class).setParameter("sid",s.getShopId()).setParameter("userId",u.getUserId()).getResultList();
//		return alist;
//	}
...
//		String jpql6="select u from Users u where userid=:userId";
//		return mgr.createQuery(jpql6, Users.class).setParameter("userId",userId).getSingleResult();
//	}
